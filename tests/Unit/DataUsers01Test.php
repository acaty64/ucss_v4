...
class DataUsers01Test extends TestCase
{
  function test_an_administrator_edit_the_datauser()
  {
    ...
    $response = $this->post("admin/datauser/update", $new_values->toArray());
    //Then 
  ...
  function test_a_docente_edit_his_datauser()
  {
    //Having a user
    $user = factory(User::class)->create();
    $datauser = factory(DataUser::class)->create(['user_id'=>$user->id, 'cdocente' => str_pad($user->id, 6, '0', STR_PAD_RIGHT)]);
